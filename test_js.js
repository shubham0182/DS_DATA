...
// ═══════════════════════════════════════
// PUSH NOTIFICATION — 30-min inactivity gap
// ═══════════════════════════════════════
...
const FEATURE_PROMOS = [
  ...
  { title:'AI Insights', body:'Upload data to get smart suggestions for null values, feature columns, and ML models.' },
  { title:'Dashboard Ready', body:'Build a custom dashboard with KPIs, AI insights, and data quality scoring.' },
  { title:'Export & Report', body:'Generate professional PDF reports with 15 sections — perfect for academic evaluation.' },
  { title:'ML Modeling', body:'Your data may be ready for ML. Try feature engineering, PCA, and model evaluation.' },
  { title:'EDA Power', body:'Explore correlations, distributions, and trends with interactive visualizations.' },
];

function sendNotification(title, body) {
  if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
    navigator.serviceWorker.controller.postMessage({
      type: 'show-return-notification', title, body
    });
...
function notifyIfLongGap() {
  const lastActive = localStorage.getItem('tsdata_last_active');
  if (!lastActive) return;
  const gap = Date.now() - parseInt(lastActive, 10);
  if (gap >= INACTIVITY_GAP_MS) {
    requestNotificationPermission();
    const msg = FEATURE_PROMOS[Math.floor(Math.random() * FEATURE_PROMOS.length)];
    sendNotification(msg.title, msg.body);
  }
}

function saveLastActive() {
  localStorage.setItem('tsdata_last_active', String(Date.now()));
}
...
window.addEventListener('pagehide', saveLastActive);
window.addEventListener('focus', notifyIfLongGap);

// ═══════════════════════════════════════
// INIT
// ═══════════════════════════════════════
document.addEventListener('DOMContentLoaded',()=>{
  // Check gap first, THEN save current timestamp
  notifyIfLongGap();
  saveLastActive();
